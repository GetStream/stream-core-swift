...
public class DefaultRequestEncoder: RequestEncoder, @unchecked Sendable {
    ...
    private func addAuthorizationHeader<T: Decodable>(
        ...
        let missingTokenError = ClientError.Unknown("Failed to get `token`, request can't be created.")
    ...
    private func encodeRequestURL<T: Decodable>(for endpoint: Endpoint<T>) throws -> URL {
        ...
        guard var url = urlComponents.url else {
            throw ClientError.Unknown("URL can't be created using components: \(urlComponents)")
        }
...
private extension URL {
    ...
    func appendingQueryItems(_ items: [URLQueryItem]) throws -> URL {
        guard var components = URLComponents(url: self, resolvingAgainstBaseURL: false) else {
            throw ClientError.Unexpected("Can't create `URLComponents` from the url: \(self).")
        }
        ...
        guard let newURL = components.url else {
            throw ClientError.Unexpected("Can't create a new `URL` after appending query items: \(items).")
        }
        return newURL
    }
}
