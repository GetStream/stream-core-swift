...
public let defaultTimeout: TimeInterval = 10

/// The default timeout value used by the `stays___` family of assertions.
public let defaultTimeoutForInversedExpecations: TimeInterval = 0.1
...
public struct Assertion {
    ...
    public let body: (_ elapsedTime: TimeInterval) -> State
    
    /// Evaluates the assertion.
    public func evaluate(elapsedTime: TimeInterval) -> State {
        body(elapsedTime)
    }
}
...
public extension Assert {
    ...
    static func willBeEqual<T: Equatable>(
        ...
        var defaultMessage: String {
            "Found difference for the expressions"
        }
